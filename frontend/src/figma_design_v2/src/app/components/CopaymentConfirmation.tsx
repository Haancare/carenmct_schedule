...
import { recipients, getReduction, getLegalDob, getCopayRate, getGradeText, getScheduleAmounts, getSchedulesForRecipient, getSchedulePeriods, useSchedulesVersion, getMonthlyLimit } from './mockData';
import type { Recipient, ServiceType, PeriodSegment } from './mockData';
...
// ── 확정 타입 ──────────────────────────────────────────────────────────
type ConfirmType = 'plan' | 'claim' | 'manual';
...
// 계획/청구 금액 = 일정 데이터 기반 (계획=전체 일정, 청구=완료 일정)
function getAmounts(r:Recipient,year:number,month:number,tab:ServiceTab):{plan:ServiceAmounts;claim:ServiceAmounts}{
  return getScheduleAmounts(r.id,year,month,tab);
}
// 해당 월·유형의 일정이 있는 수급자만 표시한다.
function getRecipientsForTab(tab:ServiceTab,year:number,month:number):Recipient[]{
  return recipients.filter(r=>r.status==='active'&&
    getSchedulesForRecipient(r.id,year,month).some(s=>s.serviceType===tab));
}
...
// ── 메인 컴포넌트 ──────────────────────────────────────────────────────
export function CopaymentConfirmation() {
  ...
  const [statusFilter,setStatusFilter]=useState<'all'|'unconfirmed'|'plan'|'claim'|'manual'>('all');
  const [confirmData,setConfirmData]=useState<AllConfirmData>({});
  // selSet stores recipientId — 선택·일괄확정은 수급자 단위로 동작한다.
  ...
  // 일정 변경 시 메모 무효화용 (단일 진실 소스 = mockData 의 schedule entry 스냅샷)
  const schedulesVer = useSchedulesVersion();

  // ── 비급여 상태 (수급자 단위) ────────────────────────────────────
  // 수급자별 비급여 명세(식사/상급/이미용 + 기타 카테고리별 금액).
  const [nonBenefitData,setNonBenefitData]=useState<AllNonBenefitData>({});
  // 요양기관 공용 기타 카테고리명 목록 (월 무관, 시설 전체에서 공유).
  const [nbOtherCategories,setNbOtherCategories]=useState<string[]>([]);
  // 비급여명세 모달 대상 수급자 ID (수급자별 단일 모달)
  const [nbModalId,setNBModalId]=useState<string|null>(null);
  // [비급여액입력] 일괄 입력 모달 — 모든 수급자에 대해 한 화면에서 처리
  const [nbBulkOpen,setNBBulkOpen]=useState(false);

  // ── 한도초과액 상태 (수급자 단위) ────────────────────────────────
  // 수급자별 한도초과액(월한도 초과분). 확정 시 입력되며 비급여처럼 수급자 단위.
  // 입력 UI는 추후 연결 예정 — 현재는 값 0(미입력).
  // outer key = "YYYY-M", inner key = recipientId
  const [limitExcessData,setLimitExcessData]=useState<Record<string,Record<string,number>>>({});

  const nbKey = `${year}-${month}`;
  const curNBMap = nonBenefitData[nbKey] ?? {};
  const getNB = (id:string): NonBenefitEntry => curNBMap[id] ?? NB_DEFAULT;
  const curLEMap = limitExcessData[nbKey] ?? {};
  // 한도초과액 = max(0, 해당 월 실제 사용액 - getMonthlyLimit 기반 월 급여한도)
  // 월중 등급 변경 시 가장 높은 등급(낮은 번호)의 한도를 적용 (getMonthlyLimit에서 처리)
  const getLimitExcess = (id:string): number => {
    if (curLEMap[id] !== undefined) return curLEMap[id];
    const r = recipients.find(x => x.id === id);
    if (!r) return 0;
    const limit = getMonthlyLimit(id, year, month) || r.monthlyLimit;
    const used = r.usedAmount ?? 0;
    return Math.max(0, used - limit);
  };

  // 수급자 비급여명세 저장 — 4종 합계가 확정 시 비급여 금액으로 사용된다.
  const saveNB = useCallback((recipientId:string, next:NonBenefitEntry) => {
    setNonBenefitData(prev=>({
      ...prev,
      [nbKey]:{
        ...(prev[nbKey]??{}),
        [recipientId]:next,
  ...
  // 일괄 비급여액 저장 — 한 번에 여러 수급자 데이터 갱신
  const saveAllNB = useCallback((next: Record<string, NonBenefitEntry>) => {
    setNonBenefitData(prev => ({
      ...prev,
      [nbKey]: { ...(prev[nbKey] ?? {}), ...next },
    }));
    setNBBulkOpen(false);
  },[nbKey]);

  const addNBCategory    = (name:string) => setNbOtherCategories(prev => prev.includes(name) ? prev : [...prev, name]);
  const removeNBCategory = (name:string) => {
    setNbOtherCategories(prev => prev.filter(c => c !== name));
    // 모든 수급자의 해당 카테고리 항목도 함께 제거
    setNonBenefitData(prev => {
      const next: AllNonBenefitData = {};
      Object.entries(prev).forEach(([k, m]) => {
        const newMap: Record<string, NonBenefitEntry> = {};
        Object.entries(m).forEach(([id, ent]) => {
          newMap[id] = { ...ent, otherItems: ent.otherItems.filter(i => i.label !== name) };
        });
        next[k] = newMap;
      });
      return next;
  ...
  // 확정 엔트리는 (수급자, 급여유형, 기간세그먼트) 단위로 저장된다.
  const entryKey=(id:string,periodKey:string)=>`${id}::${periodKey}`;
  const getEntry=useCallback((id:string,tab:ServiceTab,periodKey:string):ConfirmEntry|undefined=>
    (confirmData[`${year}-${month}-${tab}`]??{})[entryKey(id,periodKey)]
  ,[confirmData,year,month]);
  ...
  // 모든 (수급자 × 급여유형) 평탄화 — 일정이 있는 조합만 한 행으로 표시한다.
  const allRows=useMemo(()=>{
    ...
    recipients.forEach(r=>{
      ...
      SERVICE_TABS.forEach(t=>{
        const periods=getSchedulePeriods(r.id,year,month,t.key);
        periods.forEach(period=>{
          const periodKey=`${period.gradeNum}_${period.reduction}`;
          out.push({r,tab:t.key,period,periodKey,rowKey:`${r.id}::${t.key}::${periodKey}`});
        });
      });
    });
    return out;
  ...
  const monthUnconfirmed=useMemo(()=>
    Array.from({length:12},(_,i)=>{
      ...
      recipients.forEach(r=>{
        if(r.status!=='active')return;
        SERVICE_TABS.forEach(t=>{
          const periods=getSchedulePeriods(r.id,year,m,t.key);
          if(periods.length===0)return;
          const map=confirmData[`${year}-${m}-${t.key}`]??{};
          periods.forEach(p=>{
            const pk=`${p.gradeNum}_${p.reduction}`;
            if(!map[`${r.id}::${pk}`])unconf++;
          });
        });
      });
      return unconf;
  ...
  const monthTotal=useMemo(()=>
    Array.from({length:12},(_,i)=>{
      ...
      recipients.forEach(r=>{
        if(r.status!=='active')return;
        SERVICE_TABS.forEach(t=>{
          total+=getSchedulePeriods(r.id,year,m,t.key).length;
        });
      });
      return total;
    })
  ...
  const totals=useMemo(()=>{
    ...
    filtered.forEach(({r,tab,period,periodKey})=>{
      ...
      // 비급여·한도초과액은 수급자 단위 — 같은 수급자 첫 등장에서만 합산(컬럼 셀과 동일 기준).
      if(!seenNb.has(r.id)){
        seenNb.add(r.id);
        confNB+=nbTotal(getNB(r.id));
        // 확정 한도초과액은 스냅샷 기준 — 확정 엔트리가 있으면 그 값, 없으면 0
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  },[filtered,year,month,getEntry,nonBenefitData,limitExcessData,nbKey]);


  // 수급자 단위 확정 적용 — 한 수급자의 여러 행 선택을 한 번에 반영.
  // action === 'none' 인 행은 확정 해제(삭제), 그 외는 해당 기준으로 확정.
  const applyRecipientConfirm=useCallback((recipientId:string, sels:RecSelection[])=>{
    setConfirmData(prev=>{
      const next={...prev};
      sels.forEach(sel=>{
        const key=`${year}-${month}-${sel.tab}`;
        const map={...(next[key]??{})};
        const ek=entryKey(recipientId,sel.periodKey);
        if(sel.action==='none'){
          delete map[ek];
        }else{
          map[ek]={type:sel.action,count:sel.count,insuranceAmount:sel.ins,copayAmount:sel.cop,limitExcessAmount:getLimitExcess(recipientId),confirmedAt:new Date().toISOString()};
        }
        next[key]=map;
      });
      return next;
  ...
  // 선택된 수급자 일괄 확정 — 해당 수급자의 모든 행을 함께 확정(부분 확정 없음)
  const bulkConfirm=(type:'plan'|'claim')=>{
    setConfirmData(prev=>{
      const next={...prev};
      filtered.forEach(({r,tab,period,periodKey})=>{
        if(!selSet.has(r.id))return;
        const src=type==='plan'?period.plan:period.claim;
        const key=`${year}-${month}-${tab}`;
        next[key]={...(next[key]??{}),[entryKey(r.id,periodKey)]:{type,count:src.count,insuranceAmount:src.insurance,copayAmount:src.copay,limitExcessAmount:getLimitExcess(r.id),confirmedAt:new Date().toISOString()}};
      });
      return next;
    });
    setSelSet(new Set());
  };
  const bulkAllConfirm=(basis:'plan'|'claim',scope:'unconfirmed'|'all')=>{
    setConfirmData(prev=>{
      const next={...prev};
      filtered.forEach(({r,tab,period,periodKey})=>{
        const key=`${year}-${month}-${tab}`;
        const existing=(next[key]??{})[entryKey(r.id,periodKey)];
        if(scope==='unconfirmed'&&existing)return;
        const src=basis==='plan'?period.plan:period.claim;
        next[key]={...(next[key]??{}),[entryKey(r.id,periodKey)]:{type:basis,count:src.count,insuranceAmount:src.insurance,copayAmount:src.copay,limitExcessAmount:getLimitExcess(r.id),confirmedAt:new Date().toISOString()}};
      });
      return next;
    });
  ...
  const bulkCancel=()=>{
    setConfirmData(prev=>{
      const next={...prev};
      filtered.forEach(({r,tab,periodKey})=>{
        if(!selSet.has(r.id))return;
        const key=`${year}-${month}-${tab}`;
        const m={...(next[key]??{})};delete m[entryKey(r.id,periodKey)];next[key]=m;
      });
      return next;
    });
    setSelSet(new Set());
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',height:'100%',background:'#f0f4f8',overflow:'hidden'}}>
      ...
      {modalRecipientId&&(()=>{
        ...
        return (
          <RecipientConfirmModal
            r={rr}
            rows={rowsForModal}
            onApply={(sels)=>{applyRecipientConfirm(modalRecipientId,sels);setModalRecipientId(null);}}
      ...
      {nbBulkOpen && (()=>{
        ...
        return (
          <NBBulkModal
            ...
            onAddCategory={addNBCategory}
            onRemoveCategory={removeNBCategory}
            onSave={saveAllNB}
      ...
      {/* ── 비급여명세 모달 (수급자별 단일) ── */}
      {nbModalId&&(()=>{
        const r = recipients.find(x=>x.id===nbModalId);
        if(!r) return null;
        return (
          <NBDetailModal
            r={r}
            ...
            onAddCategory={addNBCategory}
            onRemoveCategory={removeNBCategory}
            onSave={(next)=>saveNB(nbModalId,next)}
