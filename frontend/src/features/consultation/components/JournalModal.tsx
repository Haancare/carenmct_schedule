...
import { getCertNo, getGuardians, getGradeNum, getGradeText } from '../utils/helpers';
...
export function JournalFormBody({ data, setData, recipientId, socialWorkerId, visitDate, writtenAt, recipient, worker, defaultStartTime, defaultEndTime }: JournalFormBodyProps) {
  const TODAY_STR = (() => {
    const d = new Date();
          ...
          <td colSpan={7} style={{...nvS, borderRight:'none'}}>
            <div style={{ display:'flex', alignItems:'center', gap:6, flexWrap:'wrap' }}>
              {(() => {
                ...
                return <>{timeSel('j_startTime', defaultStartTime || '10:00')} <span style={{fontSize:12}}>~</span> {timeSel('j_endTime', defaultEndTime || '10:30')}</>;
              })()}
...
// ── main modal ─────────────────────────────────────────────────────────────────
export function JournalModal({ visit, data, setData, status, setStatus, existing, onSave, onClose, recipient, worker }: JournalModalProps) {
  const rec = recipient ?? null;
  const sw = worker ?? null;
