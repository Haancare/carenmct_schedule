    ...
    <div style={{ background:'#fff', border:'1px solid #d1d5db', borderRadius:4, overflow:'hidden' }}>
          ...
          <td colSpan={3} style={nvS}>
            {(['최초','정기','상태변화'] as const).map((v,i)=>(
              <label key={v} style={{ fontSize:12, marginRight:10 }}>
                <input type="radio" name="nd_surveyType" checked={String(f('nd_surveyType', ''))===v} onChange={()=>{ setF('nd_surveyType',v); setF('nd_surveyEtc',''); }} style={{marginRight:3,accentColor:'#16a34a'}}/>{v}
              </label>
            ))}
            <label style={{ fontSize:12 }}>
              <input type="radio" name="nd_surveyType" checked={String(f('nd_surveyType', ''))==='기타'} onChange={()=>setF('nd_surveyType','기타')} style={{marginRight:3,accentColor:'#16a34a'}}/>기타 (<input value={String(f('nd_surveyEtc', ''))} onChange={e=>setF('nd_surveyEtc',e.target.value)} disabled={String(f('nd_surveyType', ''))!=='기타'} style={{...niS,width:80,background:String(f('nd_surveyType', ''))!=='기타'?'#f3f4f6':'#f9fbff',cursor:String(f('nd_surveyType', ''))!=='기타'?'not-allowed':undefined}}/>)
            </label>
          </td>
        </tr>
        <tr>
          ...
          <td style={nvS}>
            <div style={{ display:'flex', alignItems:'center', gap:4, flexWrap:'nowrap' }}>
              <label style={{ fontSize:12, whiteSpace:'nowrap' }}><input {...rad('nd_interviewee', '수급자')}/>수급자</label>
          ...
          <td style={nvS}>
            ...
            <label style={{ fontSize:12 }}><input {...rad('nd_mainCarer', 'N')}/>N</label>
          </td>
        </tr>
        <tr>
          <td style={nlS}>신장</td>
          <td style={nvS}>
            <div style={{ display:'flex', alignItems:'center', gap:4 }}>
              {numI('nd_height', '', 50)}<span style={{ fontSize:12, whiteSpace:'nowrap' }}>cm</span>
              <span style={{ fontSize:12, marginLeft:12, whiteSpace:'nowrap' }}>체중</span>
              {numI('nd_weight', '', 50)}<span style={{ fontSize:12, whiteSpace:'nowrap' }}>kg</span>
            </div>
          </td>
          <td style={nlS}>BMI</td>
          <td style={nvS}>{numI('nd_bmi', '', 60)}</td>
        </tr>
        <tr>
          <td style={nlS}>학력</td>
          <td colSpan={3} style={nvS}>
            {['무학·문맹','초등학교','중학교','고등학교','대학','대학원'].map(v=>(
              <label key={v} style={{ fontSize:12, marginRight:8 }}><input {...rad('nd_edu', v)}/>{v}</label>
            ))}
          </td>
        </tr>
      </tbody></table>
      ...
      <table style={ntbl}><tbody>
        <tr>
          ...
          <td style={{ ...nvS, width:'14%' }}><label style={{ fontSize:12 }}><input
            type="checkbox" checked={!!f('nd_d_noneAll', '')}
            onChange={e => {
        ...
        <tr>
          ...
          <td colSpan={2} style={nvS}>
            <label style={{ fontSize:12, marginRight:8 }}><input {...dChk('nd_d_hypertension')}/>고혈압</label>
        ...
        <tr>
          ...
          <td colSpan={2} style={nvS}>
            ...
            <label style={{ fontSize:12 }}><input {...dChk('nd_d_hyperlipid')}/>고지혈증</label>
          </td>
        </tr>
        <tr>
          <td style={nvS}>근골격계</td>
          <td colSpan={2} style={nvS}>
            <label style={{ fontSize:12, marginRight:8 }}><input {...dChk('nd_d_arthritis')}/>관절염(퇴행성 류마티스)</label>
            <label style={{ fontSize:12, marginRight:8 }}><input {...dChk('nd_d_osteoporosis')}/>골다공증</label>
            <label style={{ fontSize:12 }}><input {...dChk('nd_d_fracture')}/>골절·탈골 등 사고로 인한 후유증</label>
          </td>
        </tr>
        <tr>
        ...
        <tr>
          <td style={nvS}>감각계</td>
          <td colSpan={2} style={nvS}>
            <label style={{ fontSize:12, marginRight:8 }}><input {...dChk('nd_d_cataract')}/>백내장</label>
        ...
        <tr>
          ...
          <td colSpan={2} style={{ ...nvS, verticalAlign:'top' }}>
            <div style={{ marginBottom:4 }}>
              <span style={{ fontSize:12 }}>암(진단명: </span>{dNInp('nd_cancer', '', 200)}<span style={{ fontSize:12 }}> )</span>
            </div>
            <div style={{ marginBottom:4 }}>
            ...
            <div>
              <label style={{ fontSize:12 }}>
                <input type="checkbox" checked={!!f('nd_otherDiseaseChk', '')} onChange={e=>{ setF('nd_otherDiseaseChk',e.target.checked?1:0); setF('nd_d_noneAll',0); if(!e.target.checked) setF('nd_otherDisease',''); setIsDirty(true); }} style={{marginRight:4,accentColor:'#16a34a'}}/>기타( <input value={String(f('nd_otherDisease', ''))} onChange={e=>{setF('nd_otherDisease',e.target.value);setF('nd_d_noneAll',0);setIsDirty(true);}} disabled={!f('nd_otherDiseaseChk', '')} style={{...niS,width:160,background:!f('nd_otherDiseaseChk', '')?'#f3f4f6':'#f9fbff',cursor:!f('nd_otherDiseaseChk', '')?'not-allowed':undefined}}/> )
              </label>
            </div>
          </td>
        </tr>
      ...
      <table style={ntbl}><tbody>
        <tr>
          ...
          <td style={nvS}><label style={{ fontSize:12 }}><input
            type="checkbox" checked={!!f('nd_noMed', '')}
            onChange={e => {
        ...
        <tr>
          <td style={nvS}>
            <label style={{ fontSize:12 }}>
              <input type="checkbox" checked={!!f('nd_regMed', '')} onChange={e=>{ setF('nd_regMed',e.target.checked?1:0); setF('nd_noMed',0); if(!e.target.checked){ setF('nd_medReason',''); setF('nd_hospital',''); setF('nd_medFreq',''); } setIsDirty(true); }} style={{marginRight:4,accentColor:'#16a34a'}}/>
              정기적 약 복용(사유: <input value={String(f('nd_medReason', ''))} onChange={e=>{setF('nd_medReason',e.target.value);setF('nd_noMed',0);setIsDirty(true);}} disabled={!f('nd_regMed', '')} style={{...niS,width:140,background:!f('nd_regMed', '')?'#f3f4f6':'#f9fbff',cursor:!f('nd_regMed', '')?'not-allowed':undefined}}/>
              , 병원명: <input value={String(f('nd_hospital', ''))} onChange={e=>{setF('nd_hospital',e.target.value);setF('nd_noMed',0);setIsDirty(true);}} disabled={!f('nd_regMed', '')} style={{...niS,width:140,background:!f('nd_regMed', '')?'#f3f4f6':'#f9fbff',cursor:!f('nd_regMed', '')?'not-allowed':undefined}}/>
              , 진료주기: 월 <input value={String(f('nd_medFreq', ''))} onChange={e=>{setF('nd_medFreq',e.target.value);setF('nd_noMed',0);setIsDirty(true);}} disabled={!f('nd_regMed', '')} style={{...niS,width:30,textAlign:'center',background:!f('nd_regMed', '')?'#f3f4f6':'#f9fbff',cursor:!f('nd_regMed', '')?'not-allowed':undefined}}/> 회)
            </label>
          </td>
        </tr>
        <tr>
          <td style={nvS}>
            <label style={{ fontSize:12 }}>
              <input type="checkbox" checked={!!f('nd_irregMedChk', '')} onChange={e=>{ setF('nd_irregMedChk',e.target.checked?1:0); setF('nd_noMed',0); if(!e.target.checked) setF('nd_irregMed',''); setIsDirty(true); }} style={{marginRight:4,accentColor:'#16a34a'}}/>
              비정기적 약 복용(사유: <input value={String(f('nd_irregMed', ''))} onChange={e=>{setF('nd_irregMed',e.target.value);setF('nd_noMed',0);setIsDirty(true);}} disabled={!f('nd_irregMedChk', '')} style={{...niS,width:200,background:!f('nd_irregMedChk', '')?'#f3f4f6':'#f9fbff',cursor:!f('nd_irregMedChk', '')?'not-allowed':undefined}}/> )
            </label>
          </td>
        </tr>
        <tr>
        ...
        <tr>
          <td style={nvS}>
            <label style={{ fontSize:12 }}>
              <input type="checkbox" checked={!!f('nd_medEtcChk', '')} onChange={e=>{ setF('nd_medEtcChk',e.target.checked?1:0); setF('nd_noMed',0); if(!e.target.checked) setF('nd_medEtc',''); setIsDirty(true); }} style={{marginRight:4,accentColor:'#16a34a'}}/>기타( <input value={String(f('nd_medEtc', ''))} onChange={e=>{setF('nd_medEtc',e.target.value);setF('nd_noMed',0);setIsDirty(true);}} disabled={!f('nd_medEtcChk', '')} style={{...niS,width:200,background:!f('nd_medEtcChk', '')?'#f3f4f6':'#f9fbff',cursor:!f('nd_medEtcChk', '')?'not-allowed':undefined}}/> )
            </label>
          </td>
        </tr>
      </tbody></table>
      ...
      <table style={ntbl}><tbody>
        <tr>
          ...
          <td style={nvS}>
            <label style={{ fontSize:12, marginRight:6 }}><input type="checkbox" checked={!!f('nd_oral_good', '')}
            ...
            {(() => {
              ...
              return (<>
                <label style={{ fontSize:12, marginRight:4 }}>
                  <input type="checkbox" checked={!!f('nd_oral_denture', '')} onChange={e=>{ setF('nd_oral_denture',e.target.checked?1:0); setF('nd_oral_good',0); if(!e.target.checked){ setF('nd_oral_partial',0); setF('nd_oral_full',0); } setIsDirty(true); }} style={{marginRight:4,accentColor:'#16a34a'}}/>틀니착용
                </label>
                <span style={{ fontSize:12 }}>( </span>
                <label style={{ fontSize:12, marginRight:4 }}>
                  <input type="radio" name="nd_oral_denture_type" checked={!!f('nd_oral_partial', '')} onChange={()=>{ setF('nd_oral_partial',1); setF('nd_oral_full',0); setF('nd_oral_denture',1); setF('nd_oral_good',0); setIsDirty(true); }} disabled={!f('nd_oral_denture', '')} style={{marginRight:3,accentColor:'#16a34a',cursor:!f('nd_oral_denture', '')?'not-allowed':undefined}}/>부분
                </label>
                <label style={{ fontSize:12, marginRight:4 }}>
                  <input type="radio" name="nd_oral_denture_type" checked={!!f('nd_oral_full', '')} onChange={()=>{ setF('nd_oral_full',1); setF('nd_oral_partial',0); setF('nd_oral_denture',1); setF('nd_oral_good',0); setIsDirty(true); }} disabled={!f('nd_oral_denture', '')} style={{marginRight:3,accentColor:'#16a34a',cursor:!f('nd_oral_denture', '')?'not-allowed':undefined}}/>완전
                </label>
                <span style={{ fontSize:12 }}>)</span>
                <label style={{ fontSize:12, marginLeft:8, marginRight:6 }}><input {...oChk('nd_oral_noDent')}/>잔존치아 없음</label>
                <label style={{ fontSize:12 }}><input type="checkbox" checked={!!f('nd_oral_etcChk', '')} onChange={e=>{ setF('nd_oral_etcChk',e.target.checked?1:0); setF('nd_oral_good',0); if(!e.target.checked) setF('nd_oral_etc',''); setIsDirty(true); }} style={{marginRight:4,accentColor:'#16a34a'}}/>기타( <input value={String(f('nd_oral_etc', ''))} onChange={e=>{setF('nd_oral_etc',e.target.value);setF('nd_oral_good',0);setIsDirty(true);}} disabled={!f('nd_oral_etcChk', '')} style={{...niS,width:100,background:!f('nd_oral_etcChk', '')?'#f3f4f6':'#f9fbff',cursor:!f('nd_oral_etcChk', '')?'not-allowed':undefined}}/> )</label>
              </>);
            })()}
          </td>
        </tr>
        <tr>
          ...
          <td style={nvS}>
            <label style={{ fontSize:12, marginRight:6 }}><input type="checkbox" checked={!!f('nd_oralH_good', '')}
            ...
            {(() => {
              ...
              return (<>
                ...
                <label style={{ fontSize:12 }}><input type="checkbox" checked={!!f('nd_oralH_etcChk', '')} onChange={e=>{ setF('nd_oralH_etcChk',e.target.checked?1:0); setF('nd_oralH_good',0); if(!e.target.checked) setF('nd_oralH_etc',''); setIsDirty(true); }} style={{marginRight:4,accentColor:'#16a34a'}}/>기타( <input value={String(f('nd_oralH_etc', ''))} onChange={e=>{setF('nd_oralH_etc',e.target.value);setF('nd_oralH_good',0);setIsDirty(true);}} disabled={!f('nd_oralH_etcChk', '')} style={{...niS,width:100,background:!f('nd_oralH_etcChk', '')?'#f3f4f6':'#f9fbff',cursor:!f('nd_oralH_etcChk', '')?'not-allowed':undefined}}/> )</label>
              </>);
            })()}
          </td>
        </tr>
        <tr>
          ...
          <td style={{ ...nvS, verticalAlign:'top' }}>
            <div style={{ marginBottom:4 }}>
              <label style={{ fontSize:12, marginRight:6 }}><input {...chk('nd_meal_normal')}/>일반식</label>
            ...
            <div style={{ marginBottom:4 }}>
              ...
              <label style={{ fontSize:12, marginRight:6 }}><input type="checkbox" checked={!!f('nd_meal_txNone', '')}
            ...
            <div style={{ marginBottom:4 }}>
              ...
              <label style={{ fontSize:12, marginRight:6 }}><input type="radio" name="nd_meal_avoid" checked={String(f('nd_meal_avoid', ''))==='none'} onChange={()=>{ setF('nd_meal_avoid','none'); setF('nd_meal_avoidDetail',''); }} style={{marginRight:3,accentColor:'#16a34a'}}/>없음</label>
              <label style={{ fontSize:12, marginRight:6 }}><input type="radio" name="nd_meal_avoid" checked={String(f('nd_meal_avoid', ''))==='yes'} onChange={()=>setF('nd_meal_avoid','yes')} style={{marginRight:3,accentColor:'#16a34a'}}/>있음</label>
              <input value={String(f('nd_meal_avoidDetail', ''))} onChange={e=>{ setF('nd_meal_avoidDetail',e.target.value); if(e.target.value) setF('nd_meal_avoid','yes'); }} disabled={String(f('nd_meal_avoid', ''))!=='yes'} style={{...niS,width:160,background:String(f('nd_meal_avoid', ''))!=='yes'?'#f3f4f6':'#f9fbff',cursor:String(f('nd_meal_avoid', ''))!=='yes'?'not-allowed':undefined}}/>
            </div>
            <div>
        ...
        <tr>
          ...
          <td style={nvS}>
            <label style={{ fontSize:12, marginRight:6 }}><input
              type="radio" name="nd_nutrition" checked={String(f('nd_nutrition', ''))==='양호'}
            ...
            {(() => {
              ...
              return (<>
                ...
                <label style={{ fontSize:12 }}><input type="checkbox" checked={!!f('nd_nutr_etcChk', '')} onChange={e=>{ setF('nd_nutr_etcChk',e.target.checked?1:0); setF('nd_nutrition','불량'); if(!e.target.checked) setF('nd_nutr_etc',''); setIsDirty(true); }} style={{marginRight:4,accentColor:'#16a34a'}}/>기타( <input value={String(f('nd_nutr_etc', ''))} onChange={e=>{setF('nd_nutr_etc',e.target.value);setF('nd_nutrition','불량');setIsDirty(true);}} disabled={!f('nd_nutr_etcChk', '')} style={{...niS,width:80,background:!f('nd_nutr_etcChk', '')?'#f3f4f6':'#f9fbff',cursor:!f('nd_nutr_etcChk', '')?'not-allowed':undefined}}/> )</label>
              </>);
            })()}
            <span style={{ fontSize:12 }}> )</span>
          </td>
      ...
      {opinBox('nd_opinion2', '라. 의견 및 판단근거')}
          ...
          <table style={ntbl}>
            ...
            <tbody>
              <tr>
                ...
                <td style={vC}>{adlBtn('nd_adl_wash', '')}</td>
                <td style={cS}>3)</td>
                <td style={cS}>화장실(이동변기) 사용하기</td>
                <td style={vC}>{adlBtn('nd_adl_toilet', '')}</td>
              </tr>
              <tr>
                <td style={cS}>2) 양치질하기(틀니관리)</td>
                <td style={vC}>{adlBtn('nd_adl_brush', '')}</td>
                <td style={cS}>4)</td>
                <td style={cS}>몸 씻기</td>
                <td style={vC}>{adlBtn('nd_adl_bath', '')}</td>
              </tr>
              <tr>
                <td style={{ ...hS, verticalAlign:'middle', textAlign:'left', paddingLeft:8 }} rowSpan={2}>나. 일상생활</td>
                <td style={cS}>1) 옷 벗고 입기</td>
                <td style={vC}>{adlBtn('nd_adl_dress', '')}</td>
                <td style={cS}>3)</td>
                <td style={cS}>음식 삼키기</td>
                <td style={vC}>{adlBtn('nd_adl_swallow', '')}</td>
              </tr>
              <tr>
                <td style={cS}>2) 식사하기</td>
                <td style={vC}>{adlBtn('nd_adl_eat', '')}</td>
              ...
              <tr>
                ...
                <td style={vC}>{adlBtn('nd_iadl_clean', '')}</td>
                <td style={cS}>4)</td>
                <td style={cS}>교통수단 이용하기</td>
                <td style={vC}>{adlBtn('nd_iadl_transport', '')}</td>
              </tr>
              <tr>
                <td style={cS}>2) 식사 준비하기</td>
                <td style={vC}>{adlBtn('nd_iadl_cook', '')}</td>
                <td style={cS}>5)</td>
                <td style={cS}>약 챙겨먹기</td>
                <td style={vC}>{adlBtn('nd_iadl_med', '')}</td>
              </tr>
              <tr>
                <td style={cS}>3) 빨래하기</td>
                <td style={vC}>{adlBtn('nd_iadl_laundry', '')}</td>
                <td style={cS}>6)</td>
                <td style={cS}>물건사기</td>
                <td style={vC}>{adlBtn('nd_iadl_shop', '')}</td>
              </tr>
            </tbody>
          </table>
        );
      ...
      <table style={ntbl}><colgroup><col style={{width:'16%'}}/><col style={{width:'14%'}}/><col/></colgroup><tbody>
        <tr>
          ...
          <td style={{ ...nvS, display:'flex', flexWrap:'wrap', gap:4, alignItems:'center' }}>
            ...
            <label style={{ fontSize:12 }}><input {...jtSubChk('nd_uri_incon', 'nd_uri_good')}/>요실금</label>
        ...
        <tr>
          ...
          <td style={{ ...nvS, display:'flex', flexWrap:'wrap', gap:4, alignItems:'center' }}>
            <label style={{ fontSize:12 }}><input {...chk('nd_uri_mToilet')}/>화장실</label>
      ...
      <table style={ntbl}><colgroup><col style={{width:'16%'}}/><col style={{width:'14%'}}/><col/></colgroup><tbody>
        <tr>
          ...
          <td style={{ ...nvS, display:'flex', flexWrap:'wrap', gap:4, alignItems:'center' }}>
            <label style={{ fontSize:12 }}><input {...jtNoneChk('nd_bow_good',['nd_bow_incon','nd_bow_diarrhea','nd_bow_constip','nd_bow_noFeel','nd_bow_etcChk'],['nd_bow_etc'])}/>양호</label>
        ...
        <tr>
          ...
          <td style={{ ...nvS, display:'flex', flexWrap:'wrap', gap:4, alignItems:'center' }}>
            <label style={{ fontSize:12 }}><input {...chk('nd_bow_mToilet')}/>화장실</label>
      ...
      {opinBox('nd_opinion3', '바. 의견 및 판단근거')}
      ...
      <table style={ntbl}><tbody>
        <tr>
          ...
          <td style={nvS}>
            <label style={{ fontSize:12, marginRight:6 }}><input {...jtNoneChk('nd_jt_con_none',['nd_jt_con_uL','nd_jt_con_uR','nd_jt_con_lL','nd_jt_con_lR'],'nd_jt_con_part')}/>없음</label>
        ...
        <tr>
          ...
          <td style={nvS}>
            ...
            <span style={{ fontSize:12 }}>상지 (</span>
            <label style={{ fontSize:12, marginRight:2 }}><input {...jtSubChk('nd_jt_mov_uL', 'nd_jt_mov_none')}/>좌</label>
            <label style={{ fontSize:12 }}><input {...jtSubChk('nd_jt_mov_uR', 'nd_jt_mov_none')}/>우</label>
            <span style={{ fontSize:12 }}>) 하지 (</span>
            <label style={{ fontSize:12, marginRight:2 }}><input {...jtSubChk('nd_jt_mov_lL', 'nd_jt_mov_none')}/>좌</label>
            <label style={{ fontSize:12 }}><input {...jtSubChk('nd_jt_mov_lR', 'nd_jt_mov_none')}/>우</label>
            <span style={{ fontSize:12 }}>) 부위( </span>{jtPartInp('nd_jt_mov_part','nd_jt_mov_none','',100)}<span style={{ fontSize:12 }}> )</span>
          </td>
        </tr>
        <tr>
          <td style={nlS}>3) 마비</td>
          <td style={nvS}>
            <label style={{ fontSize:12, marginRight:6 }}><input {...jtNoneChk('nd_jt_para_none',['nd_jt_para_uL','nd_jt_para_uR','nd_jt_para_lL','nd_jt_para_lR'],'nd_jt_para_part')}/>없음</label>
            <span style={{ fontSize:12 }}>상지 (</span>
        ...
        <tr>
          ...
          <td style={nvS}>
            <label style={{ fontSize:12, marginRight:6 }}><input {...jtNoneChk('nd_jt_amp_none',['nd_jt_amp_uL','nd_jt_amp_uR','nd_jt_amp_lL','nd_jt_amp_lR'],undefined)}/>없음</label>
            <span style={{ fontSize:12 }}>상지 (</span>
        ...
        <tr>
          ...
          <td style={{ ...nvS, verticalAlign:'top' }}>
            <div style={{ marginBottom:3 }}>
              ...
              <label style={{ fontSize:12 }}><input {...rad('nd_walk_type', 'device')}/>보장구를 사용하여 자립보행 가능</label>
            </div>
            <div style={{ marginBottom:3 }}>
            ...
            <div>
              ...
              <label style={{ fontSize:12, marginRight:6 }}><input {...chk('nd_walk_walker')}/>성인용 보행기</label>
      ...
      <table style={ntbl}><tbody>
        <tr>
          ...
          <td style={nvS}>
            <label style={{ fontSize:12, marginRight:8 }}><input type="radio" name="nd_fall" checked={String(f('nd_fall', ''))==='none'} onChange={()=>{ setF('nd_fall','none'); setF('nd_fall_cnt',''); }} style={{marginRight:3,accentColor:'#16a34a'}}/>없음</label>
            <label style={{ fontSize:12 }}><input type="radio" name="nd_fall" checked={String(f('nd_fall', ''))==='yes'} onChange={()=>setF('nd_fall','yes')} style={{marginRight:3,accentColor:'#16a34a'}}/>있음(횟수: <input value={String(f('nd_fall_cnt', ''))} onChange={e=>{ setF('nd_fall_cnt',e.target.value); if(e.target.value) setF('nd_fall','yes'); }} style={{...niS,width:40,textAlign:'center'}}/> )</label>
          </td>
        </tr>
      </tbody></table>
          ...
          <table style={ntbl}>
            ...
            <tbody>
              <tr>
                ...
                <td style={vC2}>{adlBtn2('nd_phys_turn', '')}</td>
                <td style={cS2}>3)</td>
                <td style={cS2}>바닥에 앉은 상태에서 일어서기</td>
                <td style={vC2}>{adlBtn2('nd_phys_standup', '')}</td>
              </tr>
              <tr>
                <td style={cS2}>2) 누운 상태에서 몸 일으켜 앉기</td>
                <td style={vC2}>{adlBtn2('nd_phys_situp', '')}</td>
                <td style={cS2}>4)</td>
                <td style={cS2}>실내 보행하기</td>
                <td style={vC2}>{adlBtn2('nd_phys_indoorWalk', '')}</td>
              </tr>
            </tbody>
          </table>
        );
      })()}
      {opinBox('nd_opinion4', '마. 의견 및 판단근거')}
      ...
      <table style={ntbl}><tbody>
        <tr>
          ...
          <td style={nvS}>
            <label style={{ fontSize:12, marginRight:8 }}><input {...jtNoneChk('nd_nrs_resp_none',['nd_nrs_resp_trach','nd_nrs_resp_suction','nd_nrs_resp_o2','nd_nrs_resp_neb','nd_nrs_resp_etcChk'],['nd_nrs_resp_etc'])}/>없음</label>
        ...
        <tr>
          ...
          <td style={{ ...nvS, verticalAlign:'top' }}>
            <div style={{ marginBottom:2 }}>
              <label style={{ fontSize:12, marginRight:8 }}><input {...jtNoneChk('nd_nrs_skin_none',['nd_nrs_skin_ulcerChk','nd_nrs_skin_neuro','nd_nrs_skin_wound','nd_nrs_skin_burn','nd_nrs_skin_etcChk'],['nd_nrs_skin_part','nd_nrs_skin_stage','nd_nrs_skin_etc'])}/>없음</label>
            </div>
            <div style={{ marginBottom:2, display:'flex', alignItems:'center', flexWrap:'wrap', gap:4 }}>
              <label style={{ fontSize:12 }}>
                <input type="checkbox" checked={!!f('nd_nrs_skin_ulcerChk', '')} onChange={e=>{ setF('nd_nrs_skin_ulcerChk',e.target.checked?1:0); setF('nd_nrs_skin_none',0); if(!e.target.checked){ setF('nd_nrs_skin_part',''); setF('nd_nrs_skin_stage',''); } }} style={{marginRight:4,accentColor:'#16a34a'}}/>욕창
              </label>
              <span style={{ fontSize:12 }}>(부위: <input value={String(f('nd_nrs_skin_part', ''))} onChange={e=>{ setF('nd_nrs_skin_part',e.target.value); setF('nd_nrs_skin_none',0); }} disabled={!f('nd_nrs_skin_ulcerChk', '')} style={{...niS,width:100,background:!f('nd_nrs_skin_ulcerChk', '')?'#f3f4f6':'#f9fbff',cursor:!f('nd_nrs_skin_ulcerChk', '')?'not-allowed':undefined}}/></span>
              <span style={{ fontSize:12 }}>단계:</span>
              {['1','2','3','4'].map(v => (
                <label key={v} style={{ fontSize:12 }}>
                  <input type="radio" name="nd_nrs_skin_stage" checked={String(f('nd_nrs_skin_stage', ''))===v} onChange={()=>{ setF('nd_nrs_skin_stage',v); setF('nd_nrs_skin_none',0); }} disabled={!f('nd_nrs_skin_ulcerChk', '')} style={{marginRight:3,accentColor:'#16a34a',cursor:!f('nd_nrs_skin_ulcerChk', '')?'not-allowed':undefined}}/>{v}단계
                </label>
              ))}
              <span style={{ fontSize:12 }}>)</span>
            </div>
        ...
        <tr>
          ...
          <td style={nvS}>
            <label style={{ fontSize:12, marginRight:8 }}><input {...jtNoneChk('nd_nrs_gi_none',['nd_nrs_gi_ngt','nd_nrs_gi_gastro','nd_nrs_gi_etcChk'],['nd_nrs_gi_etc'])}/>없음</label>
        ...
        <tr>
          ...
          <td style={{ ...nvS, verticalAlign:'top' }}>
            ...
            <div style={{ marginTop:2, display:'flex', alignItems:'center', flexWrap:'wrap', gap:4 }}>
              <label style={{ fontSize:12 }}>
                <input type="checkbox" checked={!!f('nd_nrs_pain_genChk', '')} onChange={e=>{ setF('nd_nrs_pain_genChk',e.target.checked?1:0); setF('nd_nrs_pain_none',0); if(!e.target.checked){ setF('nd_nrs_pain_part',''); setF('nd_nrs_pain_score',''); } }} style={{marginRight:4,accentColor:'#16a34a'}}/>일반통증
              </label>
              <span style={{ fontSize:12 }}>(부위: <input value={String(f('nd_nrs_pain_part', ''))} onChange={e=>{ setF('nd_nrs_pain_part',e.target.value); setF('nd_nrs_pain_none',0); }} disabled={!f('nd_nrs_pain_genChk', '')} style={{...niS,width:100,background:!f('nd_nrs_pain_genChk', '')?'#f3f4f6':'#f9fbff',cursor:!f('nd_nrs_pain_genChk', '')?'not-allowed':undefined}}/></span>
              <span style={{ fontSize:12 }}>, 정도:</span>
              <select value={String(f('nd_nrs_pain_score', ''))} onChange={e=>{ setF('nd_nrs_pain_score',e.target.value); setF('nd_nrs_pain_none',0); }} disabled={!f('nd_nrs_pain_genChk', '')} style={{ fontSize:12, border:'1px solid #d1d5db', borderRadius:3, padding:'2px 4px', background:!f('nd_nrs_pain_genChk', '')?'#f3f4f6':'#f9fbff', cursor:!f('nd_nrs_pain_genChk', '')?'not-allowed':undefined }}>
            ...
            <div style={{ marginTop:2, display:'flex', alignItems:'center', flexWrap:'wrap', gap:4 }}>
              <label style={{ fontSize:12 }}>
                <input type="checkbox" checked={!!f('nd_nrs_pain_cancerChk', '')} onChange={e=>{ setF('nd_nrs_pain_cancerChk',e.target.checked?1:0); setF('nd_nrs_pain_none',0); if(!e.target.checked){ setF('nd_nrs_pain_cancerPart',''); setF('nd_nrs_pain_cancerScore',''); } }} style={{marginRight:4,accentColor:'#16a34a'}}/>암성통증
              </label>
              <span style={{ fontSize:12 }}>(부위: <input value={String(f('nd_nrs_pain_cancerPart', ''))} onChange={e=>{ setF('nd_nrs_pain_cancerPart',e.target.value); setF('nd_nrs_pain_none',0); }} disabled={!f('nd_nrs_pain_cancerChk', '')} style={{...niS,width:100,background:!f('nd_nrs_pain_cancerChk', '')?'#f3f4f6':'#f9fbff',cursor:!f('nd_nrs_pain_cancerChk', '')?'not-allowed':undefined}}/></span>
              <span style={{ fontSize:12 }}>, 정도:</span>
              <select value={String(f('nd_nrs_pain_cancerScore', ''))} onChange={e=>{ setF('nd_nrs_pain_cancerScore',e.target.value); setF('nd_nrs_pain_none',0); }} disabled={!f('nd_nrs_pain_cancerChk', '')} style={{ fontSize:12, border:'1px solid #d1d5db', borderRadius:3, padding:'2px 4px', background:!f('nd_nrs_pain_cancerChk', '')?'#f3f4f6':'#f9fbff', cursor:!f('nd_nrs_pain_cancerChk', '')?'not-allowed':undefined }}>
            ...
            <div style={{ marginTop:4 }}>
              ...
              <label style={{ fontSize:12 }}><input {...jtSubChk('nd_nrs_pain_weekly', 'nd_nrs_pain_none')}/>주 1회 이상</label>
            </div>
            ...
            <div style={{ display:'flex', alignItems:'center', flexWrap:'nowrap', gap:6, marginTop:2, justifyContent:'flex-end' }}>
              <label style={{ fontSize:12, whiteSpace:'nowrap' }}><input {...jtSubChk('nd_nrs_pain_oral', 'nd_nrs_pain_none')}/>일반진통제</label>
        ...
        <tr>
          ...
          <td style={{ ...nvS, display:'flex', alignItems:'center', flexWrap:'wrap', gap:6 }}>
            <label style={{ fontSize:12 }}><input {...jtNoneChk('nd_nrs_uri_none',['nd_nrs_uri_cath','nd_nrs_uri_simple','nd_nrs_uri_urostomy','nd_nrs_uri_bladder','nd_nrs_uri_dialysisChk','nd_nrs_uri_hemo','nd_nrs_uri_peritoneal','nd_nrs_uri_etcChk'],['nd_nrs_uri_etc'])}/>없음</label>
        ...
        <tr>
          ...
          <td style={nvS}>
            <label style={{ fontSize:12, marginRight:8 }}><input {...jtNoneChk('nd_nrs_bow_none',['nd_nrs_bow_stoma','nd_nrs_bow_enema','nd_nrs_bow_etcChk'],['nd_nrs_bow_etc'])}/>없음</label>
        ...
        <tr>
          ...
          <td style={nvS}>
            <label style={{ fontSize:12, marginRight:8 }}><input {...jtNoneChk('nd_nrs_endo_none',['nd_nrs_endo_insulin','nd_nrs_endo_etcChk'],['nd_nrs_endo_etc'])}/>없음</label>
      ...
      {opinBox('nd_opinion5', '아. 의견 및 판단근거')}
      ...
      <table style={ntbl}><tbody>
        <tr>
          ...
          <td style={{ ...nvS, verticalAlign:'top' }}>
            {/* 1줄: 양호 */}
            <div style={{ marginBottom:3 }}>
              <label style={{ fontSize:12 }}><input {...jtNoneChk('nd_cog_good',['nd_cog_memChk','nd_cog_shortMem','nd_cog_longMem','nd_cog_oriChk','nd_cog_time','nd_cog_place','nd_cog_person','nd_cog_judge','nd_cog_understand','nd_cog_attention'],undefined)}/>양호</label>
            </div>
            {/* 2줄: 기억력·지남력·판단력·이해력·주의력 */}
            <div style={{ display:'flex', alignItems:'center', flexWrap:'wrap', gap:6 }}>
              <label style={{ fontSize:12 }}>
                <input type="checkbox" checked={!!f('nd_cog_memChk', '')} onChange={e=>{ setF('nd_cog_memChk',e.target.checked?1:0); setF('nd_cog_good',0); if(!e.target.checked){ setF('nd_cog_shortMem',0); setF('nd_cog_longMem',0); } }} style={{marginRight:4,accentColor:'#16a34a'}}/>기억력 저하
              </label>
              <span style={{ fontSize:12 }}>(</span>
              <label style={{ fontSize:12 }}><input type="checkbox" checked={!!f('nd_cog_shortMem', '')} onChange={e=>{ setF('nd_cog_shortMem',e.target.checked?1:0); if(e.target.checked){ setF('nd_cog_good',0); setF('nd_cog_memChk',1); } }} disabled={!f('nd_cog_memChk', '')} style={{marginRight:4,accentColor:'#16a34a',cursor:!f('nd_cog_memChk', '')?'not-allowed':undefined}}/>단기</label>
              <label style={{ fontSize:12 }}><input type="checkbox" checked={!!f('nd_cog_longMem', '')} onChange={e=>{ setF('nd_cog_longMem',e.target.checked?1:0); if(e.target.checked){ setF('nd_cog_good',0); setF('nd_cog_memChk',1); } }} disabled={!f('nd_cog_memChk', '')} style={{marginRight:4,accentColor:'#16a34a',cursor:!f('nd_cog_memChk', '')?'not-allowed':undefined}}/>장기</label>
              <span style={{ fontSize:12 }}>)</span>
              <label style={{ fontSize:12 }}>
                <input type="checkbox" checked={!!f('nd_cog_oriChk', '')} onChange={e=>{ setF('nd_cog_oriChk',e.target.checked?1:0); setF('nd_cog_good',0); if(!e.target.checked){ setF('nd_cog_time',0); setF('nd_cog_place',0); setF('nd_cog_person',0); } }} style={{marginRight:4,accentColor:'#16a34a'}}/>지남력 저하
              </label>
              <span style={{ fontSize:12 }}>(</span>
              <label style={{ fontSize:12 }}><input type="checkbox" checked={!!f('nd_cog_time', '')} onChange={e=>{ setF('nd_cog_time',e.target.checked?1:0); if(e.target.checked){ setF('nd_cog_good',0); setF('nd_cog_oriChk',1); } }} disabled={!f('nd_cog_oriChk', '')} style={{marginRight:4,accentColor:'#16a34a',cursor:!f('nd_cog_oriChk', '')?'not-allowed':undefined}}/>시간</label>
              <label style={{ fontSize:12 }}><input type="checkbox" checked={!!f('nd_cog_place', '')} onChange={e=>{ setF('nd_cog_place',e.target.checked?1:0); if(e.target.checked){ setF('nd_cog_good',0); setF('nd_cog_oriChk',1); } }} disabled={!f('nd_cog_oriChk', '')} style={{marginRight:4,accentColor:'#16a34a',cursor:!f('nd_cog_oriChk', '')?'not-allowed':undefined}}/>장소</label>
              <label style={{ fontSize:12 }}><input type="checkbox" checked={!!f('nd_cog_person', '')} onChange={e=>{ setF('nd_cog_person',e.target.checked?1:0); if(e.target.checked){ setF('nd_cog_good',0); setF('nd_cog_oriChk',1); } }} disabled={!f('nd_cog_oriChk', '')} style={{marginRight:4,accentColor:'#16a34a',cursor:!f('nd_cog_oriChk', '')?'not-allowed':undefined}}/>사람</label>
              <span style={{ fontSize:12 }}>)</span>
        ...
        <tr>
          ...
          <td style={{ ...nvS, verticalAlign:'top' }}>
            {/* 1줄: 없음 */}
            <div style={{ marginBottom:3 }}>
              <label style={{ fontSize:12 }}><input {...jtNoneChk('nd_beh_none',['nd_beh_delusion','nd_beh_hallucin','nd_beh_lost','nd_beh_resist','nd_beh_irregSleep','nd_beh_pacing','nd_beh_aggressChk','nd_beh_verbal','nd_beh_nonverbal','nd_beh_inappropriate','nd_beh_dirty','nd_beh_eatingChange','nd_beh_undress','nd_beh_repeat'],undefined)}/>없음</label>
            </div>
            {/* 2줄: 나머지 모두 */}
            <div style={{ display:'flex', alignItems:'center', flexWrap:'wrap', gap:6 }}>
              ...
              <label style={{ fontSize:12 }}>
                <input type="checkbox" checked={!!f('nd_beh_aggressChk', '')} onChange={e=>{ setF('nd_beh_aggressChk',e.target.checked?1:0); setF('nd_beh_none',0); if(!e.target.checked){ setF('nd_beh_verbal',0); setF('nd_beh_nonverbal',0); } }} style={{marginRight:4,accentColor:'#16a34a'}}/>공격성
              </label>
              <span style={{ fontSize:12 }}>(</span>
              <label style={{ fontSize:12 }}><input type="checkbox" checked={!!f('nd_beh_verbal', '')} onChange={e=>{ setF('nd_beh_verbal',e.target.checked?1:0); if(e.target.checked){ setF('nd_beh_none',0); setF('nd_beh_aggressChk',1); } }} disabled={!f('nd_beh_aggressChk', '')} style={{marginRight:4,accentColor:'#16a34a',cursor:!f('nd_beh_aggressChk', '')?'not-allowed':undefined}}/>언어적</label>
              <label style={{ fontSize:12 }}><input type="checkbox" checked={!!f('nd_beh_nonverbal', '')} onChange={e=>{ setF('nd_beh_nonverbal',e.target.checked?1:0); if(e.target.checked){ setF('nd_beh_none',0); setF('nd_beh_aggressChk',1); } }} disabled={!f('nd_beh_aggressChk', '')} style={{marginRight:4,accentColor:'#16a34a',cursor:!f('nd_beh_aggressChk', '')?'not-allowed':undefined}}/>비언어적</label>
              <span style={{ fontSize:12 }}>)</span>
        ...
        <tr>
          ...
          <td style={nvS}>
            ...
            <label style={{ fontSize:12, marginRight:8 }}><input {...jtSubChk('nd_psy_depressed', 'nd_psy_none')}/>우울감</label>
        ...
        return (
          <table style={ntbl}><tbody>
            <tr>
              ...
              <td style={nvS}>{commBtn('nd_comm_understand', '',['양호','약간 어려움','단순 소통만 가능','이해 못함','측정불가'])}</td>
            </tr>
            <tr>
              <td style={nlS}>2) 표현능력</td>
              <td style={nvS}>{commBtn('nd_comm_express', '',['양호','약간 어려움','단순 표현만 가능','표현 못함','측정불가'])}</td>
            </tr>
            <tr>
              <td style={nlS}>3) 시력상태</td>
              <td style={nvS}>
                {commBtn('nd_comm_vision', '',['양호','1m거리 보임','근접한 사물 보임','보이지 않음','판단불가'])}
                <div style={{ marginTop:2 }}>
            ...
            <tr>
              ...
              <td style={nvS}>
                {commBtn('nd_comm_hearing', '',['양호','먼 곳 소리 안 들림','큰 소리만 들림','들리지 않음','판단불가'])}
                <div style={{ marginTop:2 }}>
      ...
      {opinBox('nd_opinion6', '마. 의견 및 판단근거')}
          ...
          <td style={nvS}>
            {[['own','자택'],['nursing','노인요양시설(노인요양공동생활가정 포함)'],['senior','양로시설'],['hospital','요양병원']].map(([v,l])=>(
              <label key={v} style={{ fontSize:12, marginRight:8 }}>
                <input type="radio" name="nd_home_type" checked={String(f('nd_home_type', ''))===v} onChange={()=>{ setF('nd_home_type',v); setF('nd_home_etc',''); }} style={{marginRight:3,accentColor:'#16a34a'}}/>{l}
              </label>
            ))}
            <label style={{ fontSize:12 }}>
              <input type="radio" name="nd_home_type" checked={String(f('nd_home_type', ''))==='etc'} onChange={()=>setF('nd_home_type','etc')} style={{marginRight:3,accentColor:'#16a34a'}}/>기타( <input value={String(f('nd_home_etc', ''))} onChange={e=>setF('nd_home_etc',e.target.value)} disabled={String(f('nd_home_type', ''))!=='etc'} style={{...niS,width:120,background:String(f('nd_home_type', ''))!=='etc'?'#f3f4f6':'#f9fbff',cursor:String(f('nd_home_type', ''))!=='etc'?'not-allowed':undefined}}/> )
            </label>
          </td>
        </tr>
        <tr>
          ...
          <td style={nvS}>
            <label style={{ fontSize:12, marginRight:6 }}><input {...chk('nd_cohabit_alone')}/>독거</label>
      ...
      <table style={ntbl}><tbody>
        <tr>
          ...
          <td style={nvS}>
            <label style={{ fontSize:12, marginRight:8 }}><input type="radio" name="nd_child_yn" checked={String(f('nd_child_yn', ''))==='none'} onChange={()=>{ setF('nd_child_yn','none'); setF('nd_child_son',''); setF('nd_child_daughter',''); }} style={{marginRight:3,accentColor:'#16a34a'}}/>없음</label>
            <label style={{ fontSize:12, marginRight:4 }}><input type="radio" name="nd_child_yn" checked={String(f('nd_child_yn', ''))==='has'} onChange={()=>setF('nd_child_yn','has')} style={{marginRight:3,accentColor:'#16a34a'}}/>있음</label><span style={{ fontSize:12 }}>(아들: </span><input value={String(f('nd_child_son', ''))} onChange={e=>{ setF('nd_child_son',e.target.value); if(e.target.value) setF('nd_child_yn','has'); }} style={{...niS,width:30,textAlign:'center'}}/>
            <span style={{ fontSize:12 }}> 명, 딸: </span><input value={String(f('nd_child_daughter', ''))} onChange={e=>{ setF('nd_child_daughter',e.target.value); if(e.target.value) setF('nd_child_yn','has'); }} style={{...niS,width:30,textAlign:'center'}}/><span style={{ fontSize:12 }}> 명)</span>
          </td>
        </tr>
        <tr>
          <td style={{ ...nlS, verticalAlign:'top' }}>2) 주 수발자</td>
          <td style={{ ...nvS, verticalAlign:'top' }}>
            <label style={{ fontSize:12, marginRight:8 }}><input {...jtNoneChk('nd_carer_none',['nd_carer_spouse','nd_carer_child','nd_carer_inlaw','nd_carer_sibling','nd_carer_grandchild','nd_carer_parent','nd_carer_relative','nd_carer_etcChk'],['nd_carer_etc','nd_carer_burden'])}/>없음</label>
            ...
            <div style={{ marginTop:4, marginLeft:8 }}>
              ...
              {(['noBurden','sometimes','often','selfBurden','always'] as const).map((v,i,arr)=>(
                <label key={v} style={{ fontSize:12, marginRight: i===arr.length-1?0:6 }}>
                  <input type="radio" name="nd_carer_burden" checked={String(f('nd_carer_burden', ''))===v} onChange={()=>{ setF('nd_carer_burden',v); setF('nd_carer_none',0); }} style={{marginRight:3,accentColor:'#16a34a'}}/>
        ...
        <tr>
          ...
          <td style={{ ...nvS, verticalAlign:'top' }}>
            <div style={{ marginBottom:4 }}>
              ...
              <label style={{ fontSize:12, marginRight:6 }}><input {...rad('nd_social_alone', '예')}/>예</label>
            ...
            <div style={{ marginBottom:4 }}>
              ...
              {['주1~2회','월1~2회','분기1~2회','연1~2회','없음'].map(v=>(
                <label key={v} style={{ fontSize:12, marginRight:6 }}><input {...rad('nd_social_family', v)}/>{v}</label>
              ))}
            </div>
            <div>
              <span style={{ fontSize:12, marginRight:8, display:'inline-block', width:80 }}>친구·이웃 교류</span>
              {['주1~2회','월1~2회','분기1~2회','연1~2회','없음'].map(v=>(
                <label key={v} style={{ fontSize:12, marginRight:6 }}><input {...rad('nd_social_friend', v)}/>{v}</label>
              ))}
            </div>
          </td>
        </tr>
      ...
      <table style={ntbl}><tbody>
        <tr>
          ...
          <td style={{ ...nvS, verticalAlign:'top' }}>
            <label style={{ fontSize:12, marginRight:8 }}><input {...jtNoneChk('nd_comm_resNone',['nd_comm_resCustomChk','nd_comm_resSenior','nd_comm_resHealth','nd_comm_resFood','nd_comm_resTransport','nd_comm_resHousing','nd_comm_resBeauty','nd_comm_resDisabled','nd_comm_resReligion','nd_comm_resEtcChk'],['nd_comm_resCustom','nd_comm_resEtc'])}/>없음</label>
            <div style={{ marginTop:4 }}>
      ...
      {opinBox('nd_opinion7', '라. 의견 및 판단근거')}
        ...
        <tr>
          ...
          <td style={nvS}>
            {['지층','1층','2층','3층 이상'].map(v=>(
              <label key={v} style={{ fontSize:12, marginRight:8 }}><input {...rad('nd_env_floor', v)}/>{v}</label>
            ))}
          </td>
        </tr>
      </tbody></table>
      ...
      <table style={ntbl}>
        ...
        <tbody>
          {[
            { l1:'1) 엘리베이터', k1:'nd_env_elevator', o1:['있음','없음'], d1:'', l2:'7) 주방 상태', k2:'nd_env_kitchen', o2:['양호','불량'], d2:'' },
            { l1:'2) 실내 외 계단', k1:'nd_env_stairs', o1:['있음','없음'], d1:'', l2:'8) 화장실 위치', k2:'nd_env_toiletLoc', o2:['실내','실외'], d2:'' },
            { l1:'3) 실내 장애물(문턱)', k1:'nd_env_obstacle', o1:['있음','없음'], d1:'', l2:'9) 좌변기', k2:'nd_env_seatToilet', o2:['있음','없음'], d2:'' },
            { l1:'4) 바닥 벽지', k1:'nd_env_floor2', o1:['양호','불량'], d1:'', l2:'10) 온수', k2:'nd_env_hotWater', o2:['있음','없음'], d2:'' },
            { l1:'5) 냉·난방 및 환기', k1:'nd_env_heating', o1:['양호','불량'], d1:'', l2:'11) 샤워기', k2:'nd_env_shower', o2:['있음','없음'], d2:'' },
            { l1:'6) 조명', k1:'nd_env_light', o1:['양호','불량'], d1:'', l2:'12) 세면대', k2:'nd_env_sink', o2:['있음','없음'], d2:'' },
          ].map((row, idx) => (
            <tr key={idx}>
              <td style={nvS}>{row.l1}</td>
              <td style={nvS}>{row.o1.map(o=>(<label key={o} style={{ fontSize:12, marginRight:6 }}><input {...rad(row.k1, o)}/>{o}</label>))}</td>
              <td style={nvS}>{row.l2}</td>
              <td style={nvS}>{row.o2.map(o=>(<label key={o} style={{ fontSize:12, marginRight:6 }}><input {...rad(row.k2, o)}/>{o}</label>))}</td>
            </tr>
          ))}
        </tbody>
      </table>
      {opinBox('nd_opinion8', '다. 의견 및 판단근거')}
      ...
      <table style={ntbl}><tbody>
        <tr>
          ...
          <td style={{ ...nvS, verticalAlign:'top' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:4 }}>
              <label style={{ fontSize:12 }}><input {...chk('nd_svc_bath')}/>몸 씻기(목욕)</label>
              ...
              <label style={{ fontSize:12 }}><input {...chk('nd_svc_mobility')}/>이동도움(부축, 휠체어 등)</label>
        ...
        <tr>
          ...
          <td style={{ ...nvS, verticalAlign:'top' }}>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:4 }}>
              <label style={{ fontSize:12 }}><input {...chk('nd_svc_mealPrep')}/>식사 준비 및 정리</label>
              <label style={{ fontSize:12 }}><input {...chk('nd_svc_cleaning')}/>청소·주변정돈</label>
              <label style={{ fontSize:12 }}><input {...chk('nd_svc_laundry')}/>의복 세탁 및 관리</label>
              <label style={{ fontSize:12 }}><input {...chk('nd_svc_shopping')}/>장보기</label>
              <label style={{ fontSize:12 }}><input {...chk('nd_svc_medHelp')}/>복약도움</label>
              <label style={{ fontSize:12, display:'flex', alignItems:'center', flexWrap:'wrap', gap:4 }}>
                <span style={{ display:'flex', alignItems:'center' }}>
                  <input type="checkbox" checked={!!f('nd_svc_outChk', '')} onChange={e=>{ setF('nd_svc_outChk',e.target.checked?1:0); if(!e.target.checked){ setF('nd_svc_hospital',0); setF('nd_svc_office',0); setF('nd_svc_walk',0); setF('nd_svc_outEtcChk',0); setF('nd_svc_outEtc',''); } }} style={{ marginRight:4, accentColor:'#16a34a' }}/>외출동행(
                </span>
                <input type="checkbox" checked={!!f('nd_svc_hospital', '')} onChange={e=>{ setF('nd_svc_hospital',e.target.checked?1:0); if(e.target.checked) setF('nd_svc_outChk',1); }} disabled={!f('nd_svc_outChk', '')} style={{ marginRight:3, accentColor:'#16a34a', cursor:!f('nd_svc_outChk', '')?'not-allowed':undefined }}/>병원
                <input type="checkbox" checked={!!f('nd_svc_office', '')} onChange={e=>{ setF('nd_svc_office',e.target.checked?1:0); if(e.target.checked) setF('nd_svc_outChk',1); }} disabled={!f('nd_svc_outChk', '')} style={{ marginLeft:4, marginRight:3, accentColor:'#16a34a', cursor:!f('nd_svc_outChk', '')?'not-allowed':undefined }}/>관공서
                <input type="checkbox" checked={!!f('nd_svc_walk', '')} onChange={e=>{ setF('nd_svc_walk',e.target.checked?1:0); if(e.target.checked) setF('nd_svc_outChk',1); }} disabled={!f('nd_svc_outChk', '')} style={{ marginLeft:4, marginRight:3, accentColor:'#16a34a', cursor:!f('nd_svc_outChk', '')?'not-allowed':undefined }}/>산책
                <input type="checkbox" checked={!!f('nd_svc_outEtcChk', '')} onChange={e=>{ setF('nd_svc_outEtcChk',e.target.checked?1:0); if(e.target.checked) setF('nd_svc_outChk',1); if(!e.target.checked) setF('nd_svc_outEtc',''); }} disabled={!f('nd_svc_outChk', '')} style={{ marginLeft:4, marginRight:3, accentColor:'#16a34a', cursor:!f('nd_svc_outChk', '')?'not-allowed':undefined }}/>기타( <input value={String(f('nd_svc_outEtc', ''))} onChange={e=>{ setF('nd_svc_outEtc',e.target.value); if(e.target.value) setF('nd_svc_outEtcChk',1); }} disabled={!f('nd_svc_outEtcChk', '') || !f('nd_svc_outChk', '')} style={{...niS,width:50,background:(!f('nd_svc_outEtcChk', '')||!f('nd_svc_outChk', ''))?'#f3f4f6':'#f9fbff',cursor:(!f('nd_svc_outEtcChk', '')||!f('nd_svc_outChk', ''))?'not-allowed':undefined}}/> ))
              </label>
            </div>
          </td>
        </tr>
        <tr>
          ...
          <td style={{ ...nvS, verticalAlign:'top' }}>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:4 }}>
              <label style={{ fontSize:12 }}><input {...chk('nd_svc_physTrain')}/>신체기능 훈련</label>
        ...
        <tr>
          ...
          <td style={nvS}>
            ...
            <label style={{ fontSize:11, marginRight:12 }}><input {...chk('nd_svc_emotional')}/>정서지원</label>
        ...
        <tr>
          ...
          <td style={nvS}>
            <label style={{ fontSize:11, marginRight:8 }}><input {...chk('nd_svc_healthMgmt')}/>건강관리(투약관리 및 기초건강관리 등)</label>
        ...
        <tr>
          ...
          <td style={nvS}>
            <span style={{ fontSize:11 }}>희망품목( </span>{nInp('nd_svc_welfare', '', 280)}<span style={{ fontSize:11 }}> )</span>
          </td>
        </tr>
        <tr>
          <td style={{ ...nlS, verticalAlign:'top' }}>아. 장기요양급여 외<br/>희망하는 지역사회<br/>자원</td>
          <td style={{ ...nvS, verticalAlign:'top' }}>
            <label style={{ fontSize:11, marginRight:8 }}><input {...jtNoneChk('nd_svc_commNone',['nd_svc_commHealth','nd_svc_commSenior','nd_svc_commTransport','nd_svc_commFood','nd_svc_commBeauty','nd_svc_commHousing','nd_svc_commDisabled','nd_svc_commReligion','nd_svc_commEtcChk'],['nd_svc_commEtc'])}/>없음</label>
            <div style={{ marginTop:4, display:'grid', gridTemplateColumns:'1fr 1fr', gap:4 }}>
      ...
      {opinBox('nd_opinion9', '자. 의견 및 판단근거')}

      {/* ═══ 10. 종합의견 ═══ */}
      <div style={{ fontSize:12, fontWeight:700, color:'#166534', padding:'8px 10px', borderBottom:cb }}>10. 종합의견</div>
      <div style={{ padding:'8px 10px' }}>
        <textarea rows={10} value={String(f('nd_totalOpinion', ''))} onChange={e=>setF('nd_totalOpinion',e.target.value)} placeholder="수급자의 전반적 상태, 욕구, 서비스 방향 등 종합의견을 작성하세요" style={ntS}/>
      </div>
    </div>
  );
}
