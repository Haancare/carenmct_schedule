...
import {
  SERVICE_LABELS, REDUCTION_OPTIONS,
  getApprovedAmounts, getValidFrom, getValidTo, getCertNo, getMobile,
  getGradeNum, getGradeText, getReduction, getCopayRate, getServiceTypes, getRealDob,
  toUiRecipient, type CarePlanUiRecipient,
} from '../utils/recipientHelpers';
import {
  fetchCarePlanRecipients,
  fetchAssessmentDocuments,
  fetchAssessmentDocument,
  createAssessmentDocument,
  updateAssessmentDocument,
  deleteAssessmentDocument,
} from '@/lib/api/carePlan';
import type { AssessmentDocType } from '@/lib/api/carePlan.types';
import { ensureCareWorkersLoaded } from '@/lib/api/careWorkers';
...
export function CarePlanManagement() {
  const { groups: recipientGroups, loading: recipientGroupsLoading } = useRecipientGroups();
  const [recipients, setRecipients] = useState<CarePlanUiRecipient[]>([]);
  const [entries, setEntries]       = useState<DocEntry[]>([]);
  const [loadingRecipients, setLoadingRecipients] = useState(false);
  const [loadingDocs, setLoadingDocs] = useState(false);
  const [socialWorkers, setSocialWorkers] = useState<string[]>([]);
  const [workerNameToId, setWorkerNameToId] = useState<Record<string, number>>({});
  ...
  const defaultAuthor = socialWorkers[0] || '';

  // 요양보호사(작성자) 목록
  useEffect(() => {
    let cancelled = false;
    ensureCareWorkersLoaded().then(workers => {
      if (cancelled) return;
      const names: string[] = [];
      ...
      for (const w of workers) {
        ...
        if (!Number.isNaN(idNum)) map[w.name] = idNum;
      }
      setSocialWorkers(names.length ? names : ['']);
      setWorkerNameToId(map);
      setAuthor(prev => prev || names[0] || '');
    }).catch(() => {
      if (!cancelled) setSocialWorkers(['']);
    });
    return () => { cancelled = true; };
  }, []);

  // 수급자 목록 로드 (상태·검색·그룹 변경 시)
  useEffect(() => {
    let cancelled = false;
    (async () => {
      setLoadingRecipients(true);
      try {
        const list = await fetchCarePlanRecipients({
          query: search || undefined,
          activeOnly: statusFilter === 'active',
          groupId: selectedGroup,
          subgroupId: selectedSubGroup,
        });
        if (!cancelled) setRecipients(list.map(toUiRecipient));
      } catch {
    ...
    return () => { cancelled = true; };
  }, [statusFilter, search, selectedGroup, selectedSubGroup]);

  // 수급자 또는 탭이 바뀌면 → 문서 목록·최신 항목 로드
  useEffect(() => {
    if (!selectedId) {
      setEntries([]);
    ...
    (async () => {
      setLoadingDocs(true);
      try {
        const list = await fetchAssessmentDocuments(selectedId, activeTab as AssessmentDocType);
        if (cancelled) return;
        const mapped: DocEntry[] = list.map(d => ({
          id: String(d.id),
          date: d.writtenDate,
          author: d.authorName || '',
          data: {},
        ...
        setEntries(mapped);
        if (mapped.length > 0) {
          const detail = await fetchAssessmentDocument(Number(mapped[0].id));
          if (cancelled) return;
          const formData = (detail.formData || {}) as Record<string, any>;
          setEntries(prev => prev.map((e, i) => (i === 0 ? { ...e, data: formData } : e)));
          setSelectedEntryId(mapped[0].id);
          setFormDraft({ ...formData, writeDate: detail.writtenDate });
          setAuthor(detail.authorName || mapped[0].author || defaultAuthor);
        } else {
          setSelectedEntryId(null);
  ...
  const refreshEntries = async (recipientId: string, docType: DocKey) => {
    const list = await fetchAssessmentDocuments(recipientId, docType as AssessmentDocType);
    const mapped: DocEntry[] = list.map(d => ({
      id: String(d.id),
      date: d.writtenDate,
      author: d.authorName || '',
  ...
  const patchLatestWrittenDate = (recipientId: string, docType: DocKey, date: string | null) => {
    setRecipients(prev => prev.map(r => {
      if (r.id !== recipientId) return r;
      const next = { ...r.latestWrittenDates };
      if (date) next[docType] = date;
      else delete next[docType];
      return { ...r, latestWrittenDates: next };
    }));
  };

  // 특정 항목 선택
  const selectEntry = async (entry: DocEntry) => {
    setSelectedEntryId(entry.id);
    setIsNewEntry(false);
    setIsDirty(false);
    try {
      const detail = await fetchAssessmentDocument(Number(entry.id));
      const formData = (detail.formData || {}) as Record<string, any>;
      setFormDraft({ ...formData, writeDate: detail.writtenDate });
      setAuthor(detail.authorName || entry.author || defaultAuthor);
      setEntries(prev => prev.map(e => (e.id === entry.id ? { ...e, data: formData } : e)));
    } catch {
      setFormDraft({ ...entry.data, writeDate: entry.date });
      setAuthor(entry.author || defaultAuthor);
    }
  };
  ...
  const startNewFall = (mode: 'huhn' | 'bobath') => {
    ...
    setSelectedEntryId(null);
    setIsNewEntry(true);
    setFormDraft({ writeDate: TODAY, fallMode: mode });
    setAuthor(defaultAuthor);
    setIsDirty(false);
  };
  ...
  const saveForm = async () => {
    ...
    const writtenDate = String(formDraft.writeDate || TODAY);
    const formData = { ...formDraft };
    const employeeId = workerNameToId[author] ?? null;
    try {
      if (isNewEntry) {
        const created = await createAssessmentDocument({
          recipientId: selectedId,
          docType: activeTab as AssessmentDocType,
          writtenDate,
          employeeId,
          formData,
        });
        setSelectedEntryId(String(created.id));
      } else if (selectedEntryId) {
        await updateAssessmentDocument(Number(selectedEntryId), {
          writtenDate,
          employeeId,
          formData,
        });
      ...
      setIsNewEntry(false);
      setIsDirty(false);
      await refreshEntries(selectedId, activeTab);
      patchLatestWrittenDate(selectedId, activeTab, writtenDate);
    } catch {
      alert('저장에 실패했습니다.');
    }
  };
  ...
  const deleteEntry = async (entryId: string) => {
    ...
    try {
      await deleteAssessmentDocument(Number(entryId));
      const remaining = await refreshEntries(selectedId, activeTab);
      patchLatestWrittenDate(
        selectedId,
        activeTab,
        remaining[0]?.date ?? null,
      );
      if (!isNewEntry && selectedEntryId === entryId) {
        if (remaining.length > 0) {
          await selectEntry(remaining[0]);
        } else {
          setSelectedEntryId(null);
          setIsNewEntry(false);
          setFormDraft({ writeDate: TODAY });
  ...
  const activeGroup = recipientGroups.find(g => g.id === selectedGroup) ?? null;
  ...
  // 현재 탭의 항목 배열
  const activeEntries: DocEntry[] = entries;
  // 현재 편집 중인 항목
  ...
  function renderLongTerm() {
    ...
    // 팝업 행 정의 — 이미지 기준 12행
    const COST_ROWS = [
      { k:'cr0',  label:'방문요양', grp:'visit_care',    defOn:false, defPer:'주', defCnt:0, durOpts:['방문당 240분이상','방문당 210분이상','방문당 180분이상','방문당 150분이상','방문당 120분이상','방문당 90분이상','방문당 60분이상'], defDur:'', defCost:0, defCopay:0 },
      ...
      { k:'cr3',  label:'방문목욕', grp:'visit_bath',    defOn:false, defPer:'월', defCnt:0, durOpts:['차량(가정)60분이상','차량(가정)40분이상','차량미이용60분이상','차량미이용40분이상'], defDur:'', defCost:0, defCopay:0 },
      ...
      { k:'cr5',  label:'방문간호', grp:'visit_nursing', defOn:false, defPer:'월', defCnt:0, durOpts:['15분이상 30분미만','30분이상 60분미만','60분이상'], defDur:'', defCost:0, defCopay:0 },
      ...
      { k:'cr7',  label:'주야간보호', grp:'day_care',      defOn:false, defPer:'주', defCnt:0, durOpts:['3시간이상','6시간이상','8시간이상','10시간이상','13시간초과'], defDur:'', defCost:0, defCopay:0 },
    ...
    // 항목(영역 라벨)만 유지, 욕구/목표/필요내용은 비움
    const DEFAULT_NEED_AREAS: NeedArea[] = [
      { k:'na_body', label:'신체활동지원', items:[{ desire:'', goal:'', content:'' }] },
      { k:'na_cognitive', label:'인지활동지원', items:[{ desire:'', goal:'', content:'' }] },
      { k:'na_daily', label:'일상생활지원,환경관리', items:[{ desire:'', goal:'', content:'' }] },
      { k:'na_personal', label:'개인활동지원', items:[{ desire:'', goal:'', content:'' }] },
      { k:'na_bath', label:'전문목욕', items:[{ desire:'', goal:'', content:'' }] },
      { k:'na_health', label:'건강관리', items:[{ desire:'', goal:'', content:'' }] },
      { k:'na_nursing', label:'간호처치', items:[{ desire:'', goal:'', content:'' }] },
      { k:'na_rehab', label:'기능회복훈련', items:[{ desire:'', goal:'', content:'' }] },
      { k:'na_emergency', label:'응급지원', items:[{ desire:'', goal:'', content:'' }] },
      { k:'na_env', label:'생활 및 환경관리', items:[{ desire:'', goal:'', content:'' }] },
    ];
    ...
    return (
      <div style={{ background:'#fff', border:'1px solid #d1d5db', borderRadius:4, overflow:'hidden', boxSizing:'border-box' }}>
        {/* 장기요양인정번호 — 수급자 certNo에서 파싱 */}
        {(() => {
          const raw = (sel ? getCertNo(sel) : '').replace(/^L/i, '').replace(/\s/g, '');
          const parts = raw.split('-').filter(Boolean);
          const def1 = parts[0] || (raw.length > 3 ? raw.slice(0, -3) : raw);
          const def2 = parts[1] || (raw.length > 3 ? raw.slice(-3) : '');
          return (
        <div style={{ display:'flex', alignItems:'center', padding:'8px 12px', borderBottom:cb, gap:6 }}>
          ...
          <input value={String(f('lt_regNum1', def1))} onChange={e=>setF('lt_regNum1',e.target.value.replace(/\D/g,''))} inputMode="numeric" style={{ ...iS, width:120, textAlign:'center' }}/>
          <span style={{ color:'#6b7280' }}>-</span>
          <input value={String(f('lt_regNum2', def2))} onChange={e=>setF('lt_regNum2',e.target.value.replace(/\D/g,''))} inputMode="numeric" style={{ ...iS, width:50, textAlign:'center' }}/>
        </div>
          );
        })()}
        ...
        <table style={{ width:'100%', borderCollapse:'collapse', tableLayout:'fixed' }}><tbody>
          <tr>
            ...
            <td style={{ ...vE, width:'34%' }}><input value={String(f('lt_birthDate', sel?.legalDob??''))} onChange={e=>{ const d=e.target.value.replace(/\D/g,'').slice(0,8); setF('lt_birthDate', d.length>6?d.slice(0,4)+'-'+d.slice(4,6)+'-'+d.slice(6):d.length>4?d.slice(0,4)+'-'+d.slice(4):d); }} placeholder="YYYY-MM-DD" maxLength={10} style={iS}/></td>
          </tr>
          <tr>
            <td style={lS}>장기요양등급</td>
            <td style={vS}>
              <select value={String(f('lt_grade', sel ? (getGradeText(sel).includes('인지') ? '인지지원' : (getGradeNum(sel) ? String(getGradeNum(sel)) : '')) : ''))} onChange={e=>setF('lt_grade',e.target.value)} style={{ ...iS, width:110 }}>
            ...
            <td style={lS}>발급일</td>
            <td style={vE}><input type="date" value={String(f('lt_issueDate', sel? getValidFrom(sel) : ''))} onChange={e=>setF('lt_issueDate',e.target.value)} style={{ ...iS, width:'100%', maxWidth:150 }}/></td>
          </tr>
          <tr>
            ...
            <td style={vS}>
              <div style={{ display:'flex', alignItems:'center', gap:4, flexWrap:'wrap' }}>
                ...
                <input value={String(f('lt_monthlyLimit', sel ? (() => { const a = getApprovedAmounts(sel); const v = a.care || a.day || a.bath || a.nursing || a.other || 0; return v ? v.toLocaleString() : ''; })() : ''))} onChange={e=>{ const n=e.target.value.replace(/[^\d]/g,''); setF('lt_monthlyLimit', n?Number(n).toLocaleString():''); }} inputMode="numeric" style={{ ...iS, width:100, textAlign:'right' }}/>
            ...
            <td style={vE}>
              <div style={{ display:'flex', alignItems:'center', gap:4 }}>
                <input type="date" value={String(f('lt_validStart', sel? getValidFrom(sel) : ''))} onChange={e=>setF('lt_validStart',e.target.value)} style={{ ...iS, flex:1, minWidth:0 }}/>
                <span style={{ color:'#6b7280', flexShrink:0 }}>~</span>
                <input type="date" value={String(f('lt_validEnd', sel? getValidTo(sel) : ''))} onChange={e=>setF('lt_validEnd',e.target.value)} style={{ ...iS, flex:1, minWidth:0 }}/>
              </div>
            </td>
          </tr>
          <tr>
            ...
            <td style={vS}>
              <div style={{ display:'flex', alignItems:'center', gap:4, flexWrap:'wrap' }}>
                ...
                <input value={String(f('lt_fac_normal',''))} onChange={e=>{ const n=e.target.value.replace(/[^\d]/g,''); setF('lt_fac_normal',n?Number(n).toLocaleString():''); }} inputMode="numeric" style={{ ...iS, width:70, textAlign:'right' }}/><span style={{ fontSize:12, color:'#6b7280' }}>원</span>
              </div>
            </td>
            ...
            <td style={vE}>
              <div style={{ display:'flex', alignItems:'center', gap:4, flexWrap:'wrap' }}>
                <span style={{ fontSize:12, color:'#6b7280' }}>일반</span><span style={{ fontSize:12, color:'#6b7280' }}>1일당</span>
                <input value={String(f('lt_com_normal',''))} onChange={e=>{ const n=e.target.value.replace(/[^\d]/g,''); setF('lt_com_normal',n?Number(n).toLocaleString():''); }} inputMode="numeric" style={{ ...iS, width:70, textAlign:'right' }}/><span style={{ fontSize:12, color:'#6b7280' }}>원</span>
              </div>
            </td>
          </tr>
          <tr>
            <td style={vS}>
              <div style={{ display:'flex', alignItems:'center', gap:4, flexWrap:'wrap' }}>
                ...
                <input value={String(f('lt_fac_demA',''))} onChange={e=>{ const n=e.target.value.replace(/[^\d]/g,''); setF('lt_fac_demA',n?Number(n).toLocaleString():''); }} inputMode="numeric" style={{ ...iS, width:70, textAlign:'right' }}/><span style={{ fontSize:12, color:'#6b7280' }}>원</span>
              </div>
            </td>
            <td style={vE}>
              <div style={{ display:'flex', alignItems:'center', gap:4, flexWrap:'wrap' }}>
                <span style={{ fontSize:12, color:'#6b7280' }}>치매전담형</span><span style={{ fontSize:12, color:'#6b7280' }}>1일당</span>
                <input value={String(f('lt_com_dem',''))} onChange={e=>{ const n=e.target.value.replace(/[^\d]/g,''); setF('lt_com_dem',n?Number(n).toLocaleString():''); }} inputMode="numeric" style={{ ...iS, width:70, textAlign:'right' }}/><span style={{ fontSize:12, color:'#6b7280' }}>원</span>
              </div>
            </td>
          </tr>
          <tr>
            <td style={vS}>
              <div style={{ display:'flex', alignItems:'center', gap:4, flexWrap:'wrap' }}>
                ...
                <input value={String(f('lt_fac_demB',''))} onChange={e=>{ const n=e.target.value.replace(/[^\d]/g,''); setF('lt_fac_demB',n?Number(n).toLocaleString():''); }} inputMode="numeric" style={{ ...iS, width:70, textAlign:'right' }}/><span style={{ fontSize:12, color:'#6b7280' }}>원</span>
              </div>
            </td>
            <td style={vE} />
          </tr>
        ...
        <table style={{ width:'100%', borderCollapse:'collapse', tableLayout:'fixed' }}><tbody>
          <tr>
            ...
            <td style={{ ...vS, borderRight:'none', padding:'6px 10px' }}>
              <AutoTA minRows={2} value={String(f('lt_desiredSvc',''))} onChange={e=>setF('lt_desiredSvc',e.target.value)} style={tS}/>
            </td>
          </tr>
        </tbody></table>
        ...
        <table style={{ width:'100%', borderCollapse:'collapse', tableLayout:'fixed' }}><tbody>
          <tr>
            ...
            <td style={{ ...vS, borderRight:'none', padding:'6px 10px' }}>
              <AutoTA minRows={3} value={String(f('lt_cautions',''))} onChange={e=>setF('lt_cautions',e.target.value)} style={tS}/>
            </td>
          </tr>
        </tbody></table>
        ...
        <div style={{ borderTop:cb }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'8px 12px', borderBottom:cb, background:hBg, gap:8 }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:8 }}>
              ...
              <input type="date" value={String(f('lt_costBaseDate',''))} onChange={e=>setF('lt_costBaseDate',e.target.value)} style={{ ...iS, width:130 }}/>
          ...
          <table style={{ width:'100%', borderCollapse:'collapse', tableLayout:'fixed' }}><tbody>
            <tr>
              ...
              <td style={{ ...vS, borderRight:'none' }}><input value={String(f('lt_welfareItems',''))} onChange={e=>setF('lt_welfareItems',e.target.value)} placeholder="예) 지팡이, 수동휠체어 등" style={iS}/></td>
            </tr>
          </tbody></table>
        </div>
        ...
        <table style={{ width:'100%', borderCollapse:'collapse', tableLayout:'fixed' }}><tbody>
          <tr>
            ...
            <td style={vS}><input value={String(f('lt_branchPhone',''))} onChange={e=>setF('lt_branchPhone',e.target.value)} style={iS}/></td>
            <td style={{ ...lS, width:'18%', textAlign:'center' }}>지사 담당자명</td>
            <td style={{ ...vS, borderRight:'none' }}><input value={String(f('lt_branchManager',''))} onChange={e=>setF('lt_branchManager',e.target.value)} style={iS}/></td>
          </tr>
        </tbody></table>
  ...
  function renderFall() {
    ...
    /* ── 보바스 모드 ── */
    const bobathScores = BOBATH_GROUPS.map(i => {
      const raw = f(i.k, '');
      return raw === '' || raw == null ? null : Number(raw);
    });
    const bobathAllSelected = bobathScores.every(v => v != null && !Number.isNaN(v));
    const curTotal  = bobathScores.reduce<number>((s, v) => s + (v ?? 0), 0);
    const curRisk   = bobathAllSelected ? bobathRisk(curTotal) : { l:'미선택', c:'#94a3b8', bg:'#f8fafc' };
    ...
    return (
      <div style={{ background:'#fff', border:'1px solid #e2e8f0', borderRadius:8, overflow:'hidden' }}>
        ...
        <table style={{ width:'100%', borderCollapse:'collapse' }}>
          ...
          <tbody>
            {BOBATH_GROUPS.map((grp) => {
              const curRaw = f(grp.k, '');
              const cur = curRaw === '' || curRaw == null ? null : Number(curRaw);
              return (
        ...
        <div style={{ padding:'10px 14px', borderTop:'1px solid #e2e8f0', background: curRisk.bg, display:'flex', alignItems:'center', gap:16, flexWrap:'wrap' }}>
          <div>
            <span style={{ fontSize:12, color:'#64748b', marginRight:8 }}>합산 점수</span>
            <span style={{ fontSize:20, fontWeight:700, color: curRisk.c }}>{bobathAllSelected ? `${curTotal}점` : '-'}</span>
          </div>
          <div style={{ width:1, height:32, background:'#e2e8f0' }}/>
          <span style={{ fontSize:13, fontWeight:700, color: curRisk.c, padding:'3px 14px', background:`${curRisk.c}18`, borderRadius:16, border:`1px solid ${curRisk.c}40` }}>{curRisk.l}</span>
          <div style={{ flex:1 }}/>
  ...
  function renderPressure() {
    ...
    const selectedScores = BRADEN_ITEMS.map(i => {
      const raw = f(i.k, '');
      return raw === '' || raw == null ? null : Number(raw);
    });
    const allSelected = selectedScores.every(v => v != null && !Number.isNaN(v));
    const total = selectedScores.reduce<number>((s, v) => s + (v ?? 0), 0);
    const risk  = allSelected ? bradenRisk(total) : { l:'미선택', c:'#94a3b8', bg:'#f8fafc' };
    ...
    return (
      <div style={{ background:'#fff', border:'1px solid #e2e8f0', borderRadius:8, overflow:'hidden' }}>
        ...
        <div style={{ padding:'10px 14px', borderTop:'1px solid #e2e8f0', background: risk.bg, display:'flex', alignItems:'center', gap:16, flexWrap:'wrap' }}>
          <div>
            <span style={{ fontSize:12, color:'#64748b', marginRight:8 }}>합산 점수</span>
            <span style={{ fontSize:20, fontWeight:700, color: risk.c }}>{allSelected ? `${total}점` : '-'}</span>
          </div>
          <div style={{ width:1, height:32, background:'#e2e8f0' }}/>
  ...
  function renderCarePlan() {
    ...
    return <CarePlanDocument formDraft={formDraft} setF={setF} recipient={sel} />;
  }
        ...
        <div style={{ flex:1, overflowY:'auto' }}>
          {filteredRecips.map(r => {
            ...
            const urgentDocs = PROGRESS_DOCS.filter(d => {
              const latestDate = r.latestWrittenDates?.[d.key] ?? null;
            ...
            return (
              <div key={r.id} onClick={() => { setSelectedId(r.id); setActiveTab('longTerm'); }} style={{
                ...
                {PROGRESS_DOCS.map(doc => {
                  const latestDate = r.latestWrittenDates?.[doc.key] ?? null;
        ...
        <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden' }}>
          ...
          <div style={{ background:'#fff', borderBottom:'1px solid #e2e8f0', padding:'8px 16px', display:'flex', alignItems:'center', gap:12, flexShrink:0 }}>
            ...
            <div style={{ display:'flex', gap:6, alignItems:'center' }}>
              {DOCS.map(doc => {
                const latestDate = doc.key === activeTab
                  ? (entries[0]?.date ?? sel.latestWrittenDates?.[doc.key] ?? null)
                  : (sel.latestWrittenDates?.[doc.key] ?? null);
                const count = doc.key === activeTab ? entries.length : (latestDate ? 1 : 0);
                const p = calcProg(latestDate, doc.cycDays);
                return (
                  <button key={doc.key} onClick={() => setActiveTab(doc.key)} title={`${doc.label}: ${count === 0 ? '미작성' : p.badge || '정상'} (${count}건)`}
                    ...
                    <div style={{ width:8, height:8, borderRadius:4, background: count === 0 ? '#94a3b8' : p.bar }}/>
          ...
          <div style={{ background:'#fff', borderBottom:'1px solid #e2e8f0', display:'flex', flexShrink:0, paddingLeft:8 }}>
            {DOCS.map((doc, i) => {
              ...
              const latestDate = isAct
                ? (entries[0]?.date ?? sel.latestWrittenDates?.[doc.key] ?? null)
                : (sel.latestWrittenDates?.[doc.key] ?? null);
              const count = isAct ? entries.length : (latestDate ? 1 : 0);
              const p    = calcProg(latestDate, doc.cycDays);
              const dot  = count === 0 ? '#94a3b8' : p.bar;
              return (
          ...
          <div style={{ flex:1, overflow:'auto', padding:14, background:'#f0f4f8' }}>
            <div style={{ maxWidth:800, display:'flex', flexDirection:'column', gap:10 }}>
              ...
              {/* 발급일/작성일 + 작성자 행 */}
              <div style={{ background:'#fff', border:'1px solid #e2e8f0', borderRadius:8, padding:'10px 14px', display:'flex', alignItems:'center', gap:14, flexShrink:0, flexWrap:'nowrap', minWidth:0 }}>
                {isNewEntry && (
                  <div style={{ display:'flex', alignItems:'center', gap:5, padding:'3px 10px', borderRadius:6, background:'#eff6ff', border:'1px solid #bfdbfe', whiteSpace:'nowrap', flexShrink:0 }}>
                ...
                <div style={{ display:'flex', alignItems:'center', gap:6, whiteSpace:'nowrap', flexShrink:0 }}>
                  <span style={{ fontSize:11, color:'#64748b', whiteSpace:'nowrap', flexShrink:0 }}>{activeTab === 'longTerm' ? '발급일' : activeTab === 'cognitive' ? '검사일' : '작성일'}</span>
                  <input type="date" value={String(f('writeDate', TODAY))} onChange={e => setF('writeDate', e.target.value)} style={{ ...SH.inp, width:130, flexShrink:0 }}/>
                </div>
                {activeTab !== 'longTerm' && (
                  <div style={{ display:'flex', alignItems:'center', gap:6, whiteSpace:'nowrap', flexShrink:0 }}>
                    <span style={{ fontSize:11, color:'#64748b', whiteSpace:'nowrap', flexShrink:0 }}>{activeTab === 'cognitive' ? '검사자' : '작성자'}</span>
                    <input
                      value={author}
                      onChange={e => { setAuthor(e.target.value); setIsDirty(true); }}
                      style={{ ...SH.inp, width:100, flexShrink:0 }}
                      placeholder="작성자명 입력"
                    />
                  </div>
                )}
                ...
                {isDirty && (
                  <span style={{ fontSize:11, color:'#f97316', display:'flex', alignItems:'center', gap:4, whiteSpace:'nowrap', flexShrink:0 }}>
